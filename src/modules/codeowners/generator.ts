...
import type { FileOwnership, CodeownersOptions } from '../../types/index.js';
    ...
    for (const [group, paths] of Object.entries(grouped)) {
        ...
        for (const ownership of paths) {
            ...
            if (includeStats && ownership.owners[0]) {
                ...
                lines.push(
                    `# ${topOwner.commits} commits by ${topOwner.author}`
                );
            }
...
export function compareCodeowners(
    ...
    for (const ownership of generated) {
        ...
        const suggestedOwners = ownership.owners.map((o) =>
            emailToUsername(o.email, teamMappings)
        );
