...
export const SARIF_RULES: Record<string, SarifRule> = {
  ...
  'secret/aws-key': {
    ...
    fullDescription:
      'An AWS access key ID was detected in the source code. This could lead to unauthorized access to AWS resources.',
  ...
  'secret/github-token': {
    ...
    fullDescription:
      'A GitHub personal access token was detected. This could allow unauthorized access to repositories.',
    helpUri:
  ...
  'secret/private-key': {
    ...
    fullDescription:
      'A private key was detected in the source code. Private keys should never be committed to version control.',
  ...
  'secret/generic': {
    ...
    fullDescription:
      'A high-entropy string was detected that may be a secret, password, or API key.',
  ...
  'license/copyleft': {
    ...
    fullDescription:
      'A dependency uses a copyleft license (GPL, AGPL) which may require you to open-source your code.',
  ...
  'branch/stale': {
    ...
    fullDescription:
      'This branch has not received commits for an extended period and may be abandoned.',
  ...
  'deps/vulnerable': {
    ...
    fullDescription:
      'This dependency has known security vulnerabilities. Update to a patched version.',
...
export function branchesToSarif(
  ...
  return branches.map((branch) => {
    ...
    return createSarifResult(ruleId, branch.message ?? `Branch ${branch.name} is ${branch.type}`, {
...
export function depsToSarif(
  ...
  return deps.map((dep) => {
    ...
    return createSarifResult(
      ruleId,
      dep.message !== undefined && dep.message !== ''
        ? dep.message
        : `${dep.package}@${dep.currentVersion ?? 'unknown'} is ${dep.type}${dep.latestVersion !== undefined ? ` (latest: ${dep.latestVersion})` : ''}`,
      { uri: 'package.json' }
    );
  });
}
